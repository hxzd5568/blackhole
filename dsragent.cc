...
DSRAgent::command(int argc, const char*const* argv)
{
  ...
  if (argc == 2) 
	...
	if (strcasecmp(argv[1], "startBlackholeAttackP") == 0)
	{
      	if(isBlackhole && !isActive)
      	printf("The node is a p black.\n");
      	else
	{
          printf("Passive Blackhole Attack\n");
          isBlackhole = true;
          isActive = false;
        }
	  return TCL_OK;
    ...
    if (strcasecmp(argv[1], "startBlackholeAttackA") == 0)
	{
      if(isBlackhole&& isActive)
      printf("The node is a A black.\n");
      else{
          printf("black active\n");
          isBlackhole = true;
          isActive = true;
     ...
     if (strcasecmp(argv[1], "stopBlackholeAttack") == 0)
	{
      if(!isBlackhole)
      printf("The node is not black.\n");
      else{
          printf("No black\n");
          isBlackhole = false;
                }
...
bool
DSRAgent::disturbForwarding(hdr_sr *srh)
{
  if (isBlackhole) {
  ...
  // doesn't free p.pkt
{
     ...
     sendOutPacketWithRoute(p_copy, true); */
  {
	...
	p_copy.route.setPathMember(0,net_id);
	route_cache->addRoute(p_copy.route, Scheduler::instance().clock(), net_id);
	p_copy.route.setPathMember(0,p.dest);
}
